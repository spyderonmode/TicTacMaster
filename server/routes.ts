  ...
  app.post('/api/matchmaking/join', requireAuth, async (req: any, res) => {
    try {
      ...
      if (userRoomState && userRoomState.isInGame) {
        //console.log(`🚫 User ${userId} attempted matchmaking while in active room ${userRoomState.roomId}`);
        return res.status(400).json({ 
      ...
      //console.log(`✅ User ${userId} starting matchmaking - safely cleared only their stale states`);
      userRoomStates.delete(userId);
