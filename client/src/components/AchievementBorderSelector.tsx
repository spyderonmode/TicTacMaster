...
export function AchievementBorderSelector({ user }: AchievementBorderSelectorProps) {
  ...
  const getBorderPreview = (achievementType: string) => {
    const baseStyle = "px-3 py-2 rounded-md text-white text-sm font-bold";
      ...
      case 'ultimate_veteran':
        return (
          <motion.div
            ...
            className={`${baseStyle} border-3 border-double border-orange-600 bg-gradient-to-r from-red-900/30 via-orange-800/30 to-red-900/30`}
          >
            🔥 Ultimate Veteran
          </motion.div>
        );
      case 'grandmaster':
        return (
          <motion.div
            ...
            className={`${baseStyle} border-2 border-indigo-400 bg-gradient-to-r from-indigo-900/25 via-gray-800/25 to-indigo-900/25`}
          >
            💎 Grandmaster
          </motion.div>
        );
      case 'champion':
        return (
          <motion.div
            ...
            className={`${baseStyle} border-2 border-purple-500 bg-gradient-to-r from-purple-900/20 via-blue-900/20 to-purple-900/20`}
          >
            👑 Champion
          </motion.div>
        );
      case 'legend':
        return (
          <motion.div
            ...
            className={`${baseStyle} border-2 border-orange-500 bg-orange-900/20`}
          >
            🌟 Legend
          </motion.div>
        );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="w-full max-w-md sm:max-w-lg max-h-[75vh] sm:max-h-[80vh] overflow-y-auto overflow-x-hidden mx-auto p-4">
        <DialogHeader>
                    ...
                    <CardContent className="p-4 w-full overflow-hidden">
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-start gap-3 flex-shrink-0">
                          <div className="w-12 h-12 flex-shrink-0 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-lg">
                            {achievement.icon}
                          </div>
                          <div className="text-left">
                            <h3 className="font-medium">{achievement.achievementName}</h3>
                            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 break-words whitespace-normal w-full">
