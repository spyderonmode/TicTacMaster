...
import { Crown, Shield, Castle, Swords, Clock, Calendar, Gem, Sparkles, UserPlus, UserCheck, Loader2 } from "lucide-react"; 
...
export function PlayerProfileModal({ playerId, open, onClose, currentUserId }: PlayerProfileModalProps) {
    ...
    const [errorModal, setErrorModal] = useState<{ open: boolean; title: string; message: string; type?: 'error' | 'coins' | 'warning' | 'vip' }>({
        open: false,
        title: '',
        message: '',
    });
    ...
    const { data: outgoingRequests = [] } = useQuery<any[]>({
        queryKey: ['/api/friends/requests/outgoing', currentUserId],
        enabled: open && !!currentUserId && !!playerId && playerId !== currentUserId,
        staleTime: 60000,
    });
    ...
    const hasSentRequest = outgoingRequests.some(r => r.requestedId === playerId);
    const isAlreadyFriend = friends.some(f => f.id === playerId);
    ...
    const sendFriendRequest = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['/api/friends/requests/outgoing', currentUserId] });
        },
        onError: (error: any) => {
            const errorMessage = error?.response?.data?.error || error?.message || 'Failed to send friend request';
            setErrorModal({
                open: true,
                title: errorMessage.includes('already friend') ? 'Already Friends' : errorMessage.includes('already exists') ? 'Request Pending' : 'Error',
                message: errorMessage.includes('already friend') 
                    ? 'You are already friends with this player.'
                    : errorMessage.includes('already exists')
                    ? 'You have already sent a friend request to this player.'
                    : errorMessage,
                type: errorMessage.includes('already friend') ? 'warning' : 'error',
            });
        },
    });
                                    ...
                                    <div>
                                        ...
                                        {!isOwnProfile && (
                                            <div className="mt-1 flex items-center">
                                                {isAlreadyFriend ? (
                                                    <Badge variant="outline" className="h-5 px-1.5 text-[10px] bg-green-500/10 text-green-400 border-green-500/30 gap-1">
                                                        <UserCheck className="w-2.5 h-2.5" />
                                                        {t('alreadyFriend') || 'Already Friends'}
                                                    </Badge>
                                                ) : hasSentRequest ? (
                                                    <Badge variant="outline" className="h-5 px-1.5 text-[10px] bg-yellow-500/10 text-yellow-400 border-yellow-500/30 gap-1">
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        variant="ghost"
                                                        className="h-6 w-6 p-0 hover:bg-white/10 text-yellow-400 hover:text-yellow-300 transition-all duration-300"
