...
import vipImage from '@/lib/vip.png'; // ✅ adjust path
...
const MinimalStatLine = ({ icon, label, value, valueColor }: StatLineProps) => (
  <div className="flex justify-between items-center py-0.5 border-b border-gray-700/50 last:border-b-0">
    ...
    <div className={`text-xs font-bold ${valueColor} leading-tight font-mono`}>{value}</div>
  </div>
);
...
export function UserProfileModal({
  ...
  // ✅ VIP PASS QUERY (matches your existing pattern/boolean)
  const { data: vipPassData } = useQuery<VipPassResponse>({
    queryKey: [`/api/players/${userId}/vip-status`],
    enabled: open && !!userId,
    queryFn: async () => {
      const res = await fetch(`/api/players/${userId}/vip-status`, {
        credentials: 'include',
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      ...
      <DialogContent
        className="max-w-[90%] w-full max-h-[90vh] overflow-y-auto 
                bg-zinc-950/95 p-0 text-gray-100 
                border border-amber-600/50 rounded-lg shadow-inner-xl shadow-amber-900/40
        ...
        <DialogHeader className="p-3 border-b border-amber-600/50 bg-zinc-900/60 flex flex-row items-center justify-between">
          <DialogTitle className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              {/* ✅ Name + VIP badge right after name */}
              <h2 className="text-base font-extrabold text-amber-300 leading-tight tracking-wide uppercase">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 rounded-md bg-zinc-800/50 ring-1 ring-amber-600/30">
                <h3 className="text-[10px] uppercase text-amber-400 mb-1.5 tracking-widest font-bold border-b border-amber-700/40 pb-0.5">
                  Primary Metrics
                </h3>
              ...
              <div className="p-3 rounded-md bg-zinc-800/50 ring-1 ring-amber-600/30 space-y-2">
                <h3 className="text-[10px] uppercase text-amber-400 tracking-widest font-bold border-b border-amber-700/40 pb-0.5 mb-1.5">
                  Operational Status
                </h3>
                ...
                <div className="pb-0.5">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <span className="text-[8px] text-gray-500 font-mono">
                      {getProgressInCurrentLevel(stats.wins)}/{getWinsRequiredForCurrentLevel(stats.level + 1)} (Next
                      Lv{stats.level + 1})
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-0.5 border-t border-gray-700/50">
                  <div className="flex flex-col items-start">
                    ...
                    <div className="text-base font-bold text-orange-300 leading-tight font-mono">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <div className="text-base font-bold text-pink-300 leading-tight font-mono">
