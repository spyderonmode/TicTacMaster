...
export function InviteFriendsModal({ open, onClose, roomId, roomName }: InviteFriendsModalProps) {
  ...
  // Listen for invitation responses via WebSocket
  useEffect(() => {
    const handleInvitationAccepted = (event: any) => {
      const { acceptedBy } = event.detail;
      toast({
        title: t('invitationAccepted'),
        description: `${acceptedBy.displayName || acceptedBy.username} ${t('joinedRoomSuccessfully')}`,
      });
    };

    const handleInvitationRejected = (event: any) => {
      const { rejectedBy } = event.detail;
      toast({
        title: t('invitationDeclined'),
        description: `${rejectedBy.displayName || rejectedBy.username} ${t('youDeclinedInvitation')}`,
        variant: "destructive",
      });
    };

    window.addEventListener('room_invitation_accepted', handleInvitationAccepted as EventListener);
    window.addEventListener('room_invitation_rejected', handleInvitationRejected as EventListener);

    return () => {
      window.removeEventListener('room_invitation_accepted', handleInvitationAccepted as EventListener);
      window.removeEventListener('room_invitation_rejected', handleInvitationRejected as EventListener);
    };
  }, [toast, t]);
